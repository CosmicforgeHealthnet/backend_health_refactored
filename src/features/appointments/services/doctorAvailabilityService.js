...
// const DoctorAvailabilityRepository = require("../../repositories/appointment/doctorAvailabilityRepository");
...
//   async setWeeklyAvailability(doctorId, availabilityData) {
//     try {
...
//       // 2. Check for existing availability for these days
...
//   // Add method to replace existing weekly availability
//   async replaceWeeklyAvailability(doctorId, availabilityData) {
//     try {
//       // 1. Check for duplicate days in request
//       const days = availabilityData.map((item) => item.dayOfWeek);
//       const duplicateDays = days.filter(
//         (day, index) => days.indexOf(day) !== index
//       );
...
//       // 2. Delete existing availability for these days
//       const existingAvailability =
//         await this.availabilityRepository.findByDoctorId(doctorId);
...
//       // ✅ FIX: Normalize time format to HH:MM
//       // const bookedSlots = existingAppointments
//       //   .filter(apt => apt.status !== 'cancelled')
//       //   .map(apt => this.normalizeTimeFormat(apt.appointmentTime))
//       //   .sort();

//       // Get booked time slots (excluding cancelled and expired pending appointments)
//       const bookedSlots = existingAppointments
//         .filter((apt) => {
...
//             return createdAt > tenMinutesAgo; // Only block if created within last 10 minutes
//           }

//           // NEW: Block slots for approved appointments (scheduled, completed, rescheduled)
//           if (apt.isDoctorApproved === true) {
//             return true; // Always block approved appointments
//           }

//           return true; // All other statuses block the slot
//         })
//         .map((apt) => this.normalizeTimeFormat(apt.appointmentTime))
//         .sort();
...
//       const currentTime = isToday ? this.getCurrentTime() : null;

//       // Filter out unavailable/booked slots and past time sltots
//       const availableSlots = Array.from(allSlots)
//         .filter((slot) => {
//           // ✅ Both are now in HH:MM format
//           const isNotBooked = !bookedSlots.includes(slot);
//           const isNotUnavailable = !this.isSlotUnavailable(
//             slot,
//             unavailability,
//             date
//           );
...
//       return `${hours}:${minutes}`; // Always return HH:MM format
//     }
...
//   // Add these methods to src/services/DoctorAvailabilityService.js
//   async updateAvailability(doctorId, availabilityId, updateData) {
//     try {
//       // Validate the availability belongs to the doctor
...
//     try {
//       // Validate the unavailability belongs to the doctor
//       const unavailability =
...
//   isSlotUnavailable(slot, unavailabilityPeriods, date) {
...
// ===================================
// src/services/appointment/doctorAvailabilityService.js (Updated with Timezone Support)
// ===================================
