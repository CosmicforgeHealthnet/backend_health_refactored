...
module.exports = class WalletsysytemAddOn1753464882250 {
    ...
    async up(queryRunner) {
        await queryRunner.query(`ALTER TABLE "wallet_withdrawals" ADD COLUMN IF NOT EXISTS "transferCode" character varying(255)`);
        await queryRunner.query(`COMMENT ON COLUMN "wallet_withdrawals"."transferCode" IS 'Paystack transfer code for OTP completion'`);
        await queryRunner.query(`ALTER TABLE "wallet_withdrawals" ADD COLUMN IF NOT EXISTS "transferId" bigint`);
        await queryRunner.query(`COMMENT ON COLUMN "wallet_withdrawals"."transferId" IS 'Paystack transfer ID'`);
        await queryRunner.query(`ALTER TABLE "wallet_withdrawals" ADD COLUMN IF NOT EXISTS "paystackResponse" text`);
        await queryRunner.query(`COMMENT ON COLUMN "wallet_withdrawals"."paystackResponse" IS 'Full Paystack API response (JSON)'`);
        await queryRunner.query(`ALTER TABLE "wallet_withdrawals" ADD COLUMN IF NOT EXISTS "requiresUserOtp" boolean NOT NULL DEFAULT false`);
        ...
        await queryRunner.query(`ALTER TABLE "wallet_withdrawals" ADD COLUMN IF NOT EXISTS "parentWithdrawalId" uuid`);
        ...
        await queryRunner.query(`
          DO $$
          BEGIN
            IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'wallet_withdrawals_status_enum') THEN
              IF NOT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON e.enumtypid = t.oid WHERE t.typname = 'wallet_withdrawals_status_enum' AND e.enumlabel = 'pending_otp') THEN
                ALTER TYPE "public"."wallet_withdrawals_status_enum" RENAME TO "wallet_withdrawals_status_enum_old";
                CREATE TYPE "public"."wallet_withdrawals_status_enum" AS ENUM('pending', 'processing', 'completed', 'failed', 'cancelled', 'pending_otp');
                ALTER TABLE "wallet_withdrawals" ALTER COLUMN "status" DROP DEFAULT;
                ALTER TABLE "wallet_withdrawals" ALTER COLUMN "status" TYPE "public"."wallet_withdrawals_status_enum" USING "status"::"text"::"public"."wallet_withdrawals_status_enum";
                ALTER TABLE "wallet_withdrawals" ALTER COLUMN "status" SET DEFAULT 'pending';
                DROP TYPE "public"."wallet_withdrawals_status_enum_old";
              END IF;
            ELSE
              CREATE TYPE "public"."wallet_withdrawals_status_enum" AS ENUM('pending', 'processing', 'completed', 'failed', 'cancelled', 'pending_otp');
              ALTER TABLE "wallet_withdrawals" ALTER COLUMN "status" TYPE "public"."wallet_withdrawals_status_enum" USING "status"::"text"::"public"."wallet_withdrawals_status_enum";
