...
module.exports = class PaymentANDwalletSystem1750519428619 {
    ...
    async up(queryRunner) {
        await queryRunner.query(`CREATE TYPE "public"."transactions_servicetype_enum" AS ENUM('appointment', 'subscription', 'lab_test', 'pharmacy', 'document_access', 'verification_fee', 'chat_premium', 'other')`);
        await queryRunner.query(`CREATE TYPE "public"."transactions_plantype_enum" AS ENUM('free', 'basic', 'medium', 'premium', 'one_time')`);
        await queryRunner.query(`CREATE TYPE "public"."transactions_billingcycle_enum" AS ENUM('one_time', 'monthly', 'quarterly', 'yearly')`);
        await queryRunner.query(`CREATE TYPE "public"."transactions_paymentprovider_enum" AS ENUM('flutterwave', 'paystack')`);
        await queryRunner.query(`CREATE TYPE "public"."transactions_status_enum" AS ENUM('pending', 'processing', 'completed', 'failed', 'disputed', 'refunded', 'cancelled')`);
        ...
        await queryRunner.query(`CREATE INDEX "IDX_83d7b5bff889a7117576041154" ON "transactions" ("patientId") `);
        await queryRunner.query(`CREATE INDEX "IDX_0616e035d18de9188812a5de38" ON "transactions" ("doctorId") `);
        await queryRunner.query(`CREATE INDEX "IDX_44451197b235962f0e5658660b" ON "transactions" ("paymentProvider") `);
        await queryRunner.query(`CREATE INDEX "IDX_da87c55b3bbbe96c6ed88ea7ee" ON "transactions" ("status") `);
        await queryRunner.query(`CREATE INDEX "IDX_eb87291e3c049754429850add9" ON "transactions" ("serviceType") `);
        await queryRunner.query(`CREATE INDEX "IDX_0b3d73b95be17139f98c9d8091" ON "transactions" ("planType") `);
        ...
        await queryRunner.query(`CREATE TYPE "public"."transaction_splits_type_enum" AS ENUM('appointment_fee', 'service_fee', 'vat')`);
        await queryRunner.query(`CREATE TYPE "public"."transaction_splits_recipienttype_enum" AS ENUM('doctor_wallet', 'company_wallet')`);
        await queryRunner.query(`CREATE TYPE "public"."transaction_splits_status_enum" AS ENUM('pending', 'released', 'refunded', 'held')`);
        await queryRunner.query(`CREATE TABLE "transaction_splits" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "transactionId" uuid NOT NULL, "type" "public"."transaction_splits_type_enum" NOT NULL, "originalAmount" numeric(10,2) NOT NULL, "originalCurrency" character varying(3) NOT NULL, "usdAmount" numeric(10,2) NOT NULL, "recipientType" "public"."transaction_splits_recipienttype_enum" NOT NULL, "recipientId" uuid, "status" "public"."transaction_splits_status_enum" NOT NULL DEFAULT 'pending', "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "releasedAt" TIMESTAMP, "refundedAt" TIMESTAMP, CONSTRAINT "PK_ff450f3d91d3c2764e27a3dfc15" PRIMARY KEY ("id")); COMMENT ON COLUMN "transaction_splits"."recipientId" IS 'Doctor ID for doctor_wallet, null for company_wallet'`);
        await queryRunner.query(`CREATE INDEX "IDX_e26abb9a10ed9c148afe8c6e42" ON "transaction_splits" ("transactionId") `);
        await queryRunner.query(`CREATE INDEX "IDX_a9e4ceab38f8b8e03b0a5da508" ON "transaction_splits" ("type") `);
        await queryRunner.query(`CREATE INDEX "IDX_fa1c529ac27b6cad1d38252717" ON "transaction_splits" ("recipientType") `);
        await queryRunner.query(`CREATE INDEX "IDX_7448510c7bc2093388492bdab8" ON "transaction_splits" ("status") `);
        await queryRunner.query(`CREATE TYPE "public"."user_payment_methods_lastsuccessfulprovider_enum" AS ENUM('flutterwave', 'paystack')`);
        ...
        await queryRunner.query(`CREATE INDEX "IDX_3c58ed268aafa4d1148077b855" ON "disputes" ("type") `);
        await queryRunner.query(`CREATE INDEX "IDX_af1b242c5899aad926113d1fbd" ON "disputes" ("createdAt") `);
        await queryRunner.query(`CREATE TYPE "public"."wallet_withdrawals_status_enum" AS ENUM('pending', 'processing', 'completed', 'failed', 'cancelled')`);
        await queryRunner.query(`CREATE TABLE "wallet_withdrawals" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "walletId" uuid NOT NULL, "doctorId" uuid NOT NULL, "amountUsd" numeric(10,2) NOT NULL, "requestedCurrency" character varying(3) NOT NULL, "requestedAmount" numeric(10,2) NOT NULL, "exchangeRateUsed" numeric(10,6), "bankDetails" jsonb NOT NULL, "status" "public"."wallet_withdrawals_status_enum" NOT NULL DEFAULT 'pending', "providerReference" character varying, "failureReason" text, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "processedAt" TIMESTAMP, "completedAt" TIMESTAMP, CONSTRAINT "PK_8683ec82b0640730c9453d6b859" PRIMARY KEY ("id")); COMMENT ON COLUMN "wallet_withdrawals"."bankDetails" IS 'Bank account details for this withdrawal'`);
        await queryRunner.query(`CREATE INDEX "IDX_ad29fd88b669636c93be627401" ON "wallet_withdrawals" ("walletId") `);
        await queryRunner.query(`CREATE INDEX "IDX_681130ff2c4853d604c2e08120" ON "wallet_withdrawals" ("doctorId") `);
        await queryRunner.query(`CREATE INDEX "IDX_dac744fc4dd94d79755e42a561" ON "wallet_withdrawals" ("status") `);
        await queryRunner.query(`CREATE INDEX "IDX_8830c122a4c67a23e49b66a2dd" ON "wallet_withdrawals" ("createdAt") `);
        ...
        await queryRunner.query(`ALTER TABLE "transactions" ADD CONSTRAINT "FK_50db78fa73cb72af407852b6db9" FOREIGN KEY ("paymentMethodId") REFERENCES "user_payment_methods"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE "wallet_withdrawals" ADD CONSTRAINT "FK_ad29fd88b669636c93be6274019" FOREIGN KEY ("walletId") REFERENCES "doctor_wallets"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
